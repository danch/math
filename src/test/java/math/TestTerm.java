...
public class TestTerm {
	...
	public void testSimpleTerm() {
		Term simpleTerm=new Term(new Literal(2.0), new VariableRef('x', 0), new Literal(2.0));
		double value = simpleTerm.evaluate((c, i)-> 2.0d);
	...
	public void testTermWithAddition() {
		...
		double value = addition.evaluate((c, i)-> 2.0d);
	...
	public void testTermWithMultiplication() {
		...
		double value = operator.evaluate((c, i)-> 2.0d);
	...
	public void testQuadratic_base() {
		...
		double value = firstOp.evaluate((c, i)-> 2.0d);
		Assert.assertEquals(10.0, value,0.00001);
	}

	private Addition buildBareQuadratic() {
		Term square = new Term(Optional.empty(), Optional.of(new VariableRef('x', 0)), Optional.of(new Literal(2.0)));
		Term noExp = new Term(Optional.empty(), Optional.of(new VariableRef('x', 0)), Optional.empty());
	...
	private Addition buildQuadraticWithCoefficients() {
		Term square = new Term(Optional.of(new Literal(3.0)), Optional.of(new VariableRef('x', 0)), Optional.of(new Literal(2.0)));
		Term noExp = new Term(Optional.of(new Literal(2.0)), Optional.of(new VariableRef('x', 0)), Optional.empty());
		Formula constant = new Literal(4.0);
		Addition secondOp = new Addition(noExp, constant);
		Addition firstOp = new Addition(square, secondOp);
		return firstOp;
	...
	public void testDifferentiate_bareExponent() {
		Term term = new Term(new Literal(1.0), new VariableRef('x', 0), new Literal(2.0));
		Formula dudx = term.differentiate(new VariableRef('x', 0));
	...
	public void testDifferentiate_withBaseQuadratic() {
		Formula result = buildBareQuadratic().differentiate(new VariableRef('x', 0));
	...
	public void testExponentialDifferentiat() {
		VariableRef x0 = new VariableRef('x', 0);
		...
		double value = expDoublePrime.evaluate((series, index) -> 4.0);
		assertEquals(24.0, value, 0.00001);
		Formula expTripplePrime = expDoublePrime.differentiate(x0);
		value = expTripplePrime.evaluate((series, index) -> 4.0);
	...
	public void testSum_evaluate() {
		...
		children.add(new Term(new Literal(2.0), new VariableRef('x', 0), new Literal(2.0)));
		children.add(new Term(new Literal(4.0), new VariableRef('x', 0), new Literal(1.0)));
		children.add(new Literal(4.0));
		Sum sum = new Sum(children);
		double value = sum.evaluate((series, index) -> 4.0);
	...
	public void testSum_differentiate() {
		...
		children.add(new Literal(4.0));
		Sum sum = new Sum(children);
		Formula derivative = sum.differentiate(new VariableRef('x', 0));
	...
	public void testPower_chainrule() {
		ArrayList<Formula> children = new ArrayList<>();
		children.add(new Power(new VariableRef('x', 0), new Literal(2.0)));
		children.add(new Literal(2.0));
		Sum sum = new Sum(children);
		Power power = new Power(sum, new Literal(2));
		Formula diff = power.differentiate(new VariableRef('x', 0));
		double value = diff.evaluate((series, index) -> 4.0);
	...
	public void testDivision_simple() {
		Formula numerator = new Sum(new Product(new Literal(4.0), new VariableRef('x', 0)), new Negate(new Literal(2.0)));
		Formula denominator = new Sum(new Power(new VariableRef('x', 0), new Literal(2.0)), new Literal(1.0));
		Formula division = new Division(numerator, denominator);
		Formula derivative = division.differentiate(new VariableRef('x', 0));
	}
}
