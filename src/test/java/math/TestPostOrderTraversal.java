...
public class TestPostOrderTraversal {
	...
	public void testProduct_ProductOfFunction() {
		...
		Collection<Formula> allChildren = product.postOrderTraversal();
		assertEquals(7, allChildren.size());
		assertTrue(allChildren.contains(firstTerm));
        assertTrue(allChildren.contains(secondTerm));
        assertTrue(allChildren.contains(product));
        assertTrue(allChildren.contains(new IndexedVariableRef('x', 1)));
	}
	/*
	@Test
	public void testProduct_DivisionOfProduct() {
		...
		double value = dydx0.evaluate((variableRef)-> 2.0);
	...
	public void testProduct_TwoFactors() {
		...
		double value = dudx0.evaluate((variableRef)-> 2.0);
